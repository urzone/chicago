  ...
  files.forEach(item => {
    ...
    if (fileInfo.size / (1024 * 1024) < 20) {    // less than 20MB
      ...
      result.push({
        name: songName,
        artist: arr[2],
        source: 'https://cdn.jsdelivr.net/gh/urzone/chicago' + item.slice(1),
        url: 'https://cdn.jsdelivr.net/gh/urzone/chicago' + item.slice(1),
        cover: `https://cdn.jsdelivr.net/gh/urzone/chicago/audio/${arr[2].replace(/[()\s]/g, function (match) {
          if (match === '(') {
            return '%28'; // The album name with '(' replaced with '%28'
          } else if (match === ')') {
            return '%29'; // The album name with ')' replaced with '%29'
      ...
      download += `https://cdn.jsdelivr.net/gh/urzone/chicago${item.slice(1)}\n`
      ...
      result.push({
        name: songName,
        artist: arr[2],
        source: `https://raw.githubusercontent.com/urzone/chicago/main${item.slice(1)}`,
        url: `https://raw.githubusercontent.com/urzone/chicago/main${item.slice(1)}`,
        cover: `https://cdn.jsdelivr.net/gh/urzone/chicago/audio/${arr[2].replace(/[()\s]/g, function (match) {
          if (match === '(') {
            return '%28'; // The album name with '(' replaced with '%28'
          } else if (match === ')') {
      ...
      download += `https://raw.githubusercontent.com/urzone/chicago/main${item.slice(1)}\n`
    }
  })
